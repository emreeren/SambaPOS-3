...
namespace Samba.Persistance.Data
{
    ...
    {
        public static Specification<T> EntityByName<T>(string name) where T : class, IEntity
        {
            return new DirectSpecification<T>(x => x.Name.ToLower() == name.ToLower());
        }

        public static Specification<T> EntityById<T>(int id) where T : class, IEntity
        {
            return new DirectSpecification<T>(x => x.Id == id);
        }

        public static Specification<T> EntityDuplicates<T>(T entity) where T : class, IEntity
        {
